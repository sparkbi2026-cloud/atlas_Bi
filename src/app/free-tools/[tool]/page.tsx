...
export async function generateMetadata({ params }: { params: Promise<{ tool: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://atlasbi.live/free-tools/${tool.slug}`,
    },
  };
}

export default async function FreeToolPage({ params }: { params: Promise<{ tool: string }> }) {
  const { tool: slug } = await params;
  const tool = getToolBySlug(slug);
  if (!tool) notFound();
            ...
            "itemListElement": [
              {
                ...
                "position": 1,
                "name": "Home",
                "item": "https://atlasbi.live"
              },
              {
                "@type": "ListItem",
                "position": 2,
                "name": "Free Tools",
                "item": "https://atlasbi.live/free-tools"
              },
              {
                "@type": "ListItem",
                "position": 3,
                "name": tool.title,
                "item": `https://atlasbi.live/free-tools/${tool.slug}`
              }
            ]
          })
        }}
